/**
 * Telegram Bot - 只发送消息，不 polling（避免 409 冲突）
 * 用 fetch API 直接调用 Telegram API
 */
...
async function scanAndPushAddress(address) {
  ...
  await sendTelegram(`🔍 正在扫描链上 ${address.slice(0,10)}... 的转入记录...`);
...
module.exports = { init, scanAndPushAddress, notifyNewTransfers, sendTestMessage };
